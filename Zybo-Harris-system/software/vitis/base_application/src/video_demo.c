...
#define ISP_VDMA_ID				XPAR_ISP_AXI_VDMA_0_DEVICE_ID

#define DYNCLK_BASEADDR 		XPAR_HDMI_OUT_INTERFACE_AXI_DYNCLK_0_BASEADDR
#define VDMA_ID 				XPAR_ZYNQ_PS_AXI_VDMA_0_DEVICE_ID
#define HDMI_OUT_VTC_ID 		XPAR_HDMI_OUT_INTERFACE_V_TC_OUT_DEVICE_ID
#define HDMI_IN_VTC_ID 			XPAR_HDMI_IN_INTERFACE_V_TC_IN_DEVICE_ID
#define HDMI_IN_GPIO_ID 		XPAR_ZYNQ_PS_AXI_GPIO_VIDEO_DEVICE_ID
#define HDMI_IN_VTC_IRPT_ID 	XPAR_FABRIC_HDMI_IN_INTERFACE_V_TC_IN_IRQ_INTR
#define HDMI_IN_GPIO_IRPT_ID 	XPAR_FABRIC_ZYNQ_PS_AXI_GPIO_VIDEO_IP2INTC_IRPT_INTR
...
XAxiVdma_Config *ISPvdmaConfig;
XAxiVdma ISPvdma;
ISPVDMACtrl ISPvdmaReadController;
ISPVDMACtrl ISPvdmaWriteController;
#define ISP_DEMO_MAX_FRAME (1920*1080)
float radius = 0.5;
unsigned short threshold = 500;

u8 ISPbuf[ISP_DEMO_MAX_FRAME];
...
void DemoInitialize()
{
	...
	//MODIFICATIONS
	InitISPRegisters();
	ConfigureISPBAR((u32) pFrames[1]);
	ConfigureISPResolution(1920, 1080);
	ConfigureISPDetectionParams(radius, threshold);

	ConfigureOVRBAR((u32) pFrames[1]);
	ConfigureOVRResolution(1920, 1080);
	ConfigureOVRControl(1);
	...
	ISPvdmaConfig = XAxiVdma_LookupConfig(ISP_VDMA_ID);
	if (!ISPvdmaConfig)
	{
		xil_printf("No video ISP DMA found for ID %d\r\n", ISP_VDMA_ID);
		return;
	}
	Status = XAxiVdma_CfgInitialize(&ISPvdma, ISPvdmaConfig, ISPvdmaConfig->BaseAddress);
	if (Status != XST_SUCCESS)
	{
		xil_printf("ISP VDMA Configuration Initialization failed %d\r\n", Status);
		return;
	}
	...
	ISPvdmaReadController.vdmaConfig.FrameDelay = 0;
	ISPvdmaReadController.vdmaConfig.EnableCircularBuf = 1;
	ISPvdmaReadController.vdmaConfig.EnableSync = 0;
	ISPvdmaReadController.vdmaConfig.PointNum = 0;
	ISPvdmaReadController.vdmaConfig.EnableFrameCounter = 0;

	ISPvdmaReadController.vdmaConfig.VertSizeInput = 1080;
	ISPvdmaReadController.vdmaConfig.HoriSizeInput = 1920 * 3;
	ISPvdmaReadController.vdmaConfig.FixedFrameStoreAddr = 0;

	ISPvdmaReadController.vdmaConfig.Stride = 1920 * 3;
	ISPvdmaReadController.vdmaConfig.FrameStoreStartAddr[0] = (u32) pFrames[0];
	ISPvdmaReadController.vdma = &ISPvdma;


	Status = XAxiVdma_DmaConfig(ISPvdmaReadController.vdma, XAXIVDMA_READ, &(ISPvdmaReadController.vdmaConfig));
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Read channel config failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_DmaSetBufferAddr(ISPvdmaReadController.vdma, XAXIVDMA_READ, ISPvdmaReadController.vdmaConfig.FrameStoreStartAddr);
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Read channel set buffer address failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_DmaStart(ISPvdmaReadController.vdma, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Start read transfer failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_StartParking(ISPvdmaReadController.vdma, 0, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
...
void DemoRun()
{
	int nextFrame = 0;
	char userInput = 0;
	char StringUserIn[32];
	int StringPtr = 0;
	int fgetComplete = 0;
	unsigned short MaxISPResponse;
	/* Flush UART FIFO */
	...
	while (userInput != 'q')
	{
		...
		case '3':
			...
			MaxISPResponse = GetISPCornerMaxResponse();

			//xil_printf does not support '%f' print
			int whole, thousandths;
			whole = radius;
			thousandths = (radius - whole) * 1000;

			xil_printf("\n");
			xil_printf("enter new radius value (float) current val : %d.%3d\n", whole, thousandths);
			xil_printf("-> ");
			StringPtr = 0;
			while (XUartPs_IsReceiveData(UART_BASEADDR))
			{
			...
			while (fgetComplete == 0)
			{
				if(XUartPs_IsReceiveData(UART_BASEADDR)){
					StringUserIn[StringPtr] = XUartPs_ReadReg(UART_BASEADDR, XUARTPS_FIFO_OFFSET);
					xil_printf("%c", StringUserIn[StringPtr]);
					if(StringUserIn[StringPtr] == '\r')
						fgetComplete = 1;
					StringPtr++;
				}
			}
			radius = strtof(StringUserIn, NULL);

			xil_printf("\n");
			xil_printf("enter new threshold value (unsigned short) current val : %d\r\n ", threshold);
			xil_printf("for information, current maximum response is : %d\r\n ", MaxISPResponse);
			xil_printf("-> ");
			StringPtr = 0;
			fgetComplete = 0;
			while (XUartPs_IsReceiveData(UART_BASEADDR))
			...
			threshold = strtoul(StringUserIn, NULL, 10);
			ConfigureISPDetectionParams(radius, threshold);
...
void DemoPrintMenu()
{
	...
	xil_printf("3 - Change corner detection parameters\n\r");
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
	xil_printf("\n\r");
