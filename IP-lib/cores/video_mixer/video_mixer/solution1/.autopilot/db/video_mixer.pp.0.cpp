...
# 2 "<built-in>" 2
# 1 "src/video_mixer.cpp" 2
# 1 "src/video_mixer.h" 1
# 1 "src/video_mixer_utils.h" 1
# 1 "src/generics_n_options.h" 1
# 2 "src/video_mixer_utils.h" 2
...
# 6 "src/video_mixer_utils.h" 2





typedef ap_uint<1*8> channel_data_slv;
typedef ap_uint<1*8*3> video_data_slv;

typedef ap_axiu<1*8*3,1,0,0> video_stream_element;
typedef hls::stream<video_stream_element> video_stream;

typedef ap_axiu<1*8,1,0,0> overlay_stream_element;
typedef hls::stream<overlay_stream_element> overlay_stream;

const video_data_slv corner_code = 0x00FFFF;
const video_data_slv border_code = 0xFFFFFF;
typedef ap_uint<12> param_type_1;
# 2 "src/video_mixer.h" 2
__attribute__((sdx_kernel("VideoMixerUnit", 0))) void VideoMixerUnit(video_stream &video_stream_in,overlay_stream &overlay_stream_in,
...
# 3 "src/video_mixer.cpp" 2
__attribute__((sdx_kernel("VideoMixerUnit", 0))) void VideoMixerUnit(video_stream &video_stream_in,overlay_stream &overlay_stream_in,
     video_stream &video_stream_out, param_type_1 image_h, param_type_1 image_w){
#pragma HLS TOP name=VideoMixerUnit
# 4 "src/video_mixer.cpp"
...
void VideoMixer(video_stream &video_stream_in,overlay_stream &overlay_stream_in,
 ...
 VITIS_LOOP_25_1: for(param_type_1 i = 0; i < image_h; i++){
  VITIS_LOOP_26_2: for(param_type_1 j =0 ; j < image_w; j++){
#pragma HLS PIPELINE
